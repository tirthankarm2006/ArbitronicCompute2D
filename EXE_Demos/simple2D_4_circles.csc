...
void drawSquare(vec2 centre, float length, vec3 color);
...
void Compute(){
  ...
  drawSquare(squareCentre, squareLength, vec3(0.4, 0.9, 0.4));
}
...
void drawSquare(vec2 centre, float length, vec3 color){
if(UV.x <= centre.x + length/2 && UV.x >= centre.x - length/2 && UV.y <= centre.y + length/2 && UV.y >= centre.y - length/2)
   VALUE = vec4(color, 1.0);
}